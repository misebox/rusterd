...
名前も値も同じ 7 つです。どれも省略でき、省略することが既定値の指定です。
...
| 密 | `-D`, `--dense` | `dense` | — | なし |
| 形 | `-a`, `--aspect <w:h>` | `aspect` | `幅:高さ` の比 | `1:1` |
...
**形** は図が目指す縦横比で、`幅:高さ` と書きます。画面なら `1:1`、スライドなら
`16:9`、紙なら `210:297`。約束ではなく目標です。比率に対して広すぎる行は次の行へ
折り返されます。図が自分の形について打てる手はそれだけです。すでに指定した形から
A4 の比率以内に収まっている図には手を付けません。`@hint.level` で位置を指定した
配置も、どれだけ横長になっても書かれたとおりに描きます。
